...
# Hàm nhận dạng trên ảnh và hiển thị kết quả
import re  # Thêm import để sử dụng biểu thức chính quy
...
def result_image():
    if image_path:
        ...
        # Trích xuất nhãn từ kết quả nhận dạng
        labels = extract_labels(results)

        # Trích xuất tên file từ đường dẫn
        raw_file_name = os.path.basename(image_path).split('.')[0].lower()

        # Loại bỏ các ký tự như (1), (2), v.v. khỏi tên tệp
        file_name = re.sub(r'\s*\(\d+\)\s*', '', raw_file_name)

        # Danh sách các lớp
        classes = ['deo khau trang', 'deo khau trang sai cach', 'khau trang', 'ko deo khau trang', 'undefined']

        # Kiểm tra xem tên file có khớp với bất kỳ nhãn nào không
        if file_name in labels:
            status = "Nhận Dạng Đúng"
        else:
            status = "Nhận Dạng Sai"

        # Hiển thị kết quả
        show_image_with_results(image, results)
        update_status(f" Nhãn Chương Trình Nhận Dạng là : {labels} {status}")



# Hàm trích xuất nhãn từ kết quả nhận dạng
def extract_labels(results):
    labels = set()  # Dùng set để tránh nhãn trùng lặp
    for result in results:
        ...
        for box in boxes:
            ...
            if confidence >= confidence_threshold:
                ...
                labels.add(label)
    return list(labels)  # Trả về danh sách nhãn
