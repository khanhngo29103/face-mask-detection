...
# Hàm vẽ kết quả lên ảnh với màu sắc và kích thước theo yêu cầu
# Hàm vẽ kết quả lên ảnh với màu sắc và kích thước theo yêu cầu
        ...
        for box in boxes:
            ...
            if confidence >= confidence_threshold:
                ...
                if label == 'deo khau trang':
                    ...
                    color = (255, 0, 255)  # Hồng cho các nhãn không xác định
                ...
                # Tính toán kích thước chữ dựa trên chiều rộng của khung
                box_width = x2 - x1
                # Giả định rằng chiều cao chữ nên bằng khoảng 1/15 chiều rộng của khung
                font_scale = max(0.5, min(box_width / 300, 2))  # Giới hạn từ 0.5 đến 2
                font_thickness = max(1, int(font_scale))

                # Tính kích thước văn bản để tạo nền cho chữ
                (text_width, text_height), baseline = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                                                                      font_thickness)

                # Vị trí văn bản: góc trên bên trái của khung
                text_x = x1
                ...
                cv2.putText(image, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 255, 255),
...
def confirm_mode():
    ...
    if selected_option == "chọn chế độ nhận dạng":
        ...
        return  # Dừng lại nếu không có chế độ nào được chọn
    refresh_app()  # Gọi hàm làm mới nếu có chế độ đã chọn
    clear_frames()
...
# Frame cho các điều khiển cho nhận dạng từ hình ảnh
